...
function excerpt(candidate: RetrievalCandidate, query: string): string {
...
function isExpired(content: string, today = new Date()): boolean {
  const metadata = frontmatter(content);
...
function pathBoost(candidate: RetrievalCandidate, query: string): number {
...
export function applyRetrievalPolicy(
  ...
  const bestByPath = new Map<string, { candidate: RetrievalCandidate; score: number }>();
