...
import { parseSummaryResponse } from "./session-format.js";
...
export function createOllamaClient(config) {
    return {
        async ensureModel() {
            ...
            if (!response.ok)
                throw new Error(`Ollama unavailable at ${config.ollamaHost}`);
            const data = (await response.json());
            const found = (data.models || []).some((model) => [model.name, model.model].some((name) => name === config.summaryModel || name?.split(":")[0] === config.summaryModel.split(":")[0]));
            if (found)
                return;
