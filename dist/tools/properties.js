...
export function registerPropertiesTools(server) {
    server.tool("obsidian_properties", "List properties in the vault or read a specific property value from a file", {
        action: z
            .enum(["list", "read"])
    ...
    }, { readOnlyHint: true }, async ({ action, name, file, path, total, counts, sort, format }) => {
