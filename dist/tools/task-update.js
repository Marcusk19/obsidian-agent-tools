...
export function registerTaskUpdateTools(server) {
    server.tool("obsidian_task_update", "Toggle, complete, or uncomplete a task", {
        action: z
    ...
    }, { destructiveHint: true }, async ({ action, file, path, line, ref, daily, status }) => {
        const args = {};
        if (action === "toggle")
            args.toggle = true;
        if (action === "done")
