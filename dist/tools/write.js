...
export function registerWriteTools(server) {
    server.tool("obsidian_write", "Create a new note, or append/prepend content to a note or daily note", {
        action: z
    ...
    }, { destructiveHint: true }, async ({ action, content, file, path, name, overwrite, template, inline }) => {
        const args = { content };
        if (file)
            args.file = file;
        if (path)
