...
export function registerGraphTools(server) {
    server.tool("obsidian_graph", "Navigate the vault graph: backlinks, outgoing links, orphans (no incoming links), deadends (no outgoing links), or unresolved links", {
        action: z
    ...
    }, { readOnlyHint: true }, async ({ action, file, path, total, verbose, counts, all, format }) => {
        const args = {};
        if (file)
            args.file = file;
        if (path)
