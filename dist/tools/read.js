...
export function registerReadTools(server) {
    server.tool("obsidian_read", "Read note contents, daily note, file info, or outline. Actions: read (note content), daily:read (today's daily note), daily:path (daily note path), daily:recent (last N daily notes for cross-session context), outline (headings), file (file metadata)", {
        action: z
    ...
    }, { readOnlyHint: true }, async ({ action, file, path, format, days }) => {
        if (action === "daily:recent") {
            ...
            try {
                ...
                return { content: [{ type: "text", text: "(could not determine daily note path)" }] };
            }
            ...
            if (!dateMatch) {
                return { content: [{ type: "text", text: `(unexpected daily note path format: ${dailyPath})` }] };
            }
