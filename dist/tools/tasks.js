...
export function registerTasksTools(server) {
    server.tool("obsidian_tasks", "List tasks in the vault. Filter by status (todo/done), file, or daily note", {
        filter: z
            .enum(["todo", "done"])
            .optional()
    ...
    }, { readOnlyHint: true }, async ({ filter, file, path, daily, total, verbose, status, format }) => {
        const args = {};
        if (filter === "todo")
            args.todo = true;
        if (filter === "done")
