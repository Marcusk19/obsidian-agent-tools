...
export function registerVaultTools(server) {
    server.tool("obsidian_vault_info", "Get vault information: overview, file listing, folder listing, recent files, or bookmarks", {
        action: z
    ...
    }, { readOnlyHint: true }, async ({ action, folder, ext, total }) => {
        const args = {};
        if (folder)
            args.folder = folder;
        if (ext)
