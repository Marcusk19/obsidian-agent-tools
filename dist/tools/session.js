...
export function registerSessionTools(server) {
    server.tool("obsidian_session", `Session management shortcuts for Claude Code workflows.
...
Returns structured markdown. No configuration required; uses OBSIDIAN_SESSIONS_FILE env var (default: claude-sessions.md) for session tracking.`, {
    ...
    }, { readOnlyHint: true }, async ({ action, days, file, path, query, depth, max_notes, include }) => {
        const today = new Date().toISOString().slice(0, 10);
        if (action === "seed") {
            if (!file && !path && !query) {
                return {
                    content: [{ type: "text", text: "Error: 'seed' action requires 'file', 'path', or 'query' parameter." }],
                    isError: true,
                };
            }
            ...
            if (!resolvedFile && !resolvedPath && query) {
                try {
                    ...
                    if (lines.length === 0) {
                        return {
                            content: [{ type: "text", text: `No notes found matching query: "${query}"` }],
                            isError: true,
                        };
                    }
                    ...
                    return {
                        content: [{ type: "text", text: `Search failed for query: "${query}"` }],
                        isError: true,
                    };
                }
            }
            ...
            const seen = new Set();
            let notesRead = 0;
            const sections = [`## Graph Context — [[${seedLabel}]]`];
            // Read seed note
            try {
                ...
                return {
                    content: [{ type: "text", text: `Error: could not read seed note '${seedLabel}'.` }],
                    isError: true,
                };
            }
            // Helper to read a list of notes within budget
            const readNotes = async (paths) => {
                const results = [];
            ...
            const parsePaths = (output) => output.trim().split("\n").map(l => l.trim()).filter(Boolean);
            let backlinkPaths = [];
            let outlinkPaths = [];
            // Get backlinks
        ...
        if (action === "context") {
            const sections = [`## Session Context — ${today}`];
        ...
        const sections = [`## Morning Briefing — ${today}`];
        // Recent daily notes
        try {
            let dailyPath;
            try {
