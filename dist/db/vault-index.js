...
export const VAULT_INDEX_FINGERPRINT = createHash("sha256")
    ...
    .digest("hex");
export function vaultIndexPath(dataDir) {
    return join(dataDir, VAULT_INDEX_FILENAME);
}
export function openVaultIndex(dataDir) {
...
function metaValue(db, key) {
    return db.prepare("SELECT value FROM vault_index_meta WHERE key = ?").get(key)?.value;
}
function dropDerivedSchema(db) {
    db.exec(`
...
export function closeVaultIndex(db) {
    if (db.open)
        db.close();
}
