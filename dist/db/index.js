...
const DATA_DIR = process.env.OBSIDIAN_DATA_DIR ||
...
function runMigrations(db) {
    ...
    try {
        ...
        const srcMigrationsDir = join(dirname(dirname(thisDir)), "src", "db", "migrations");
        try {
 ...
 */
export function indexSummary(id, sessionId, date, cwd, topic, content, embedding) {
    const db = getDb();
    const now = new Date().toISOString().replace(/\.\d{3}Z$/, "Z");
    // Insert into canonical table
    db.prepare(`INSERT OR REPLACE INTO summaries (id, session_id, date, cwd, topic, content, created_at)
     VALUES (?, ?, ?, ?, ?, ?, ?)`).run(id, sessionId, date, cwd, topic, content, now);
    // Insert into FTS5
    db.prepare(`INSERT INTO summary_fts (content, topic, session_id, cwd, date, created_at)
     VALUES (?, ?, ?, ?, ?, ?)`).run(content, topic, sessionId, cwd, date, now);
    // Insert into vec (best-effort)
    if (embedding) {
        const vecJson = JSON.stringify(embedding);
        db.prepare(`INSERT INTO summary_vec (embedding, content, topic, session_id, cwd, date, created_at)
       VALUES (?, ?, ?, ?, ?, ?, ?)`).run(vecJson, content, topic, sessionId, cwd, date, now);
        const vecRowid = db.prepare("SELECT last_insert_rowid() as id").get().id;
        db.prepare(`INSERT OR REPLACE INTO summary_vec_map (summary_id, vec_rowid)
       VALUES (?, ?)`).run(id, vecRowid);
    }
}
