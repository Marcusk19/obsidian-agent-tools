...
export async function retrieveMemoryContext(request, config, dependencies = {}) {
    const prompt = request.prompt?.trim();
    if (!prompt)
        return { candidates: [], rendered: "", truncated: false };
    ...
    const collectTier = async (tier, limit, pathPrefixes, options = {}) => {
        if (limit <= 0)
            return [];
        try {
            ...
            for (const result of results) {
                if (seen.has(result.path) || (options.confirmedOnly && result.confidence !== "confirmed"))
                    continue;
                selected.push({
                    path: result.path,
                    title: result.title,
