...
export function renderMemoryContext(candidates, maxChars) {
    if (!candidates.length || maxChars <= 0)
        return { rendered: "", truncated: false };
    const limit = Math.max(1, maxChars);
    const sorted = [...candidates].sort((a, b) => TIER_ORDER[a.tier] - TIER_ORDER[b.tier]
        || CONFIDENCE_ORDER[a.confidence] - CONFIDENCE_ORDER[b.confidence]
        || a.score - b.score);
    let rendered = "## Relevant memory";
    let truncated = false;
    if (rendered.length > limit)
        return { rendered: rendered.slice(0, limit), truncated: true };
